...
class AccountConsolidationCheck(models.TransientModel):
    ...
    state = fields.Selection(
        [("open", "Open"), ("error", "Error"), ("ok", "Checks ok")], default="open"
    )
    ...
    def check_interco_partner(self):
        ...
        if invalid_partners:
            ...
            for partner, company in invalid_partners.items():
                err_lines.append("<li>%s : %s</li>" % (partner.name, company.name))
    ...
    def check_companies_allowed(self):
        ...
        if unallowed_companies:
            ...
            for comp in unallowed_companies:
                err_lines.append(
                    "<li>%s : User has no access to this company.</li>" % (comp.name)
                )
            err_lines.append("</ul></div>")
        return err_lines
    ...
    def check_configuration(self):
        ...
        if mapping_errors:
            messages.append(
                _("<h3>Invalid account mapping</h3>") + "".join(mapping_errors)
            )
        if partner_errors:
            messages.append(
                _("<h3>Company defined on intercompany partners</h3>")
                + "".join(partner_errors)
            )
        if company_allowed_errors:
            messages.append(
                _("<h3>Company access not allowed</h3>")
                + "".join(company_allowed_errors)
            )

        if messages:
            self.message = _("<h2>Consolidation configuration errors</h2>") + "".join(
                messages
            )
            self.state = "error"
        else:
