...
MOCK_PATH = "odoo.addons.account_consolidation_oca"
...
class TestAccountConsolidation(TransactionCase):
    ...
    def test_default_values(self):
        ...
        self.assertEqual(
            wizard.journal_id, self.holding.consolidation_default_journal_id
        )
        self.assertEqual(
            wizard.consolidation_profile_ids.mapped("sub_company_id"),
    ...
    def test_consolidation_checks_error_unallowed_company(self):
        self.env.user.write(
            {"company_ids": [(6, False, [self.holding.id, self.subsidiary_a.id])]}
        )
        wizard = self.env["account.consolidation.check"].create({})
        wizard.check_configuration()
        self.assertEqual(wizard.state, "error")
    ...
    def test_consolidation_jan_with_exchange_rates(self):
        ...
        for line in conso_move_lines:
            ...
            self.assertAlmostEqual(line.balance, conso_results[acc], places=2)
    ...
    def test_consolidation_jan_with_interco_partner(self):
        ...
        interco_partners = self.subsidiary_a.partner_id | self.subsidiary_b.partner_id

        interco_conso_move_lines = conso_move_lines.filtered(
            lambda line: line.consol_partner_id in interco_partners
        )

        conso_results = {
            "subA": {"rev1": -100, "rec1": 100},
            "subB": {
        ...
        for line in interco_conso_move_lines:
            ...
            self.assertAlmostEqual(
                line.balance, conso_results[conso_comp][acc], places=2
            )
    ...
    def test_multi_consolidation_(self):
        ...
        for jan_move in jan_moves:
            self.assertTrue(jan_move.reversed_entry_id)
        ...
        jan_reversal_lines_values = jan_reversals.mapped("line_ids").read(line_fields)
        ...
        new_feb_moves_lines_values = new_feb_moves.mapped("line_ids").read(line_fields)
        ...
        _compare_new_read_values(
            jan_reversal_lines_values, new_jan_reversal_lines_values, line_fields
        )
        _compare_new_read_values(feb_moves_values, new_feb_moves_values, entry_fields)
        _compare_new_read_values(
        ...
        with mock.patch(
            ...
            self.assertTrue(all(new_feb_moves.mapped("auto_post")))
            self.assertFalse(new_feb_moves.mapped("reversed_entry_id"))
