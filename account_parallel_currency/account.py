...
class account_account(orm.Model):
    ...
    def _build_account_vals(self, cr, uid, account_vals, parallel_company, context=None):
        ...
        if 'parent_id' in account_vals:
            parent_account = self.browse(
                cr, uid, account_vals['parent_id'], context)
    ...
    def write(self, cr, uid, ids, vals, context=None):
        if 'parallel_account_ids' not in vals:
            # write/create parallel accounts only if 'parallel_account_ids' not
            # explicity written
            for acc_id in ids:
        ...
        res = super(account_account, self).write(
    ...
    def unlink(self, cr, uid, ids, context=None):
        ...
        res = super(account_account, self).unlink(
...
class account_move(orm.Model):
    ...
    def lines_balance(self, move_lines):
        ...
        for line_tuple in move_lines:
            balance += line_tuple[2]['debit'] or (-
                                                  line_tuple[2]['credit']) or 0.0
        return balance
        ...
        if curr_pool.is_zero(cr, uid, curr_pool.browse(cr, uid, currency_id), balance):
            ...
            for line_tuple in move_lines:
                account = acc_pool.browse(
                    cr, uid, line_tuple[2]['account_id'], context)
                # search for liquidity, receivable or payable accounts
                # beacause usually they are the result of operations (invoice total).
                # So, if we made the invoice in parallel currency, we'd get
                # that different invoice total
    ...
    def post(self, cr, uid, ids, context=None):
        ...
        for move in self.browse(cr, uid, ids, context=context):
            if move.state == 'posted' and not move.parallel_move_ids:
                # avoid double post in case of 'Skip Draft State for Manual
                # Entries'
                new_move_lines = []
                parallel_data = {}
                for line in move.line_id:
                    for parallel_account in line.account_id.parallel_account_ids:
                        ...
                        period_ids = self.pool.get('account.period').search(cr, uid, [
                            ...
                            ('company_id', '=',
                             parallel_account.company_id.id),
                        ])
                        ...
                        if parallel_sec_curr_iso_code != parallel_account.company_id.currency_id.name:
                            ...
                            # search parallel currency by ISO code and parallel
                            # company
                            ...
                            # compute parallel base amount from document
                            # currency, using move date
                        ...
                        if line.tax_code_id and line.tax_amount:
                            ...
                            new_line_values[
                                'tax_code_id'] = parallel_tax_code_ids[0]
                            total_tax = new_line_values[
                                'debit'] - new_line_values['credit']
                        ...
                        new_move_lines.append(
                            (parallel_account.company_id.id, (0, 0, new_line_values)))
                ...
                for company_id in parallel_data:
                    ...
                    move_lines = self.balance_lines(cr, uid, move_lines, company_pool.browse(
                        cr, uid, company_id).currency_id.id, context=context)
                    move_values = {
...
class account_tax_code(orm.Model):
    ...
    def _build_tax_code_vals(self, cr, uid, tax_code_vals, parallel_company, context=None):
        ...
        if tax_code_vals.has_key('parent_id'):
            parent_tax_code = self.browse(
                cr, uid, tax_code_vals['parent_id'], context)
    ...
    def write(self, cr, uid, ids, vals, context=None):
        if not vals.has_key('parallel_tax_code_ids'):
            # write/create parallel tax codes only if 'parallel_tax_code_ids'
            # not explicity written
        ...
        res = super(account_tax_code, self).write(
            cr, uid, ids, vals, context=context)
        return res
